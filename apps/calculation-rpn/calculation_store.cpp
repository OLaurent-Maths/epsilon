...
Calculation * CalculationStore::push(const char * text, Context * context) {
  ...
  m_startIndex++;
  if (m_startIndex >= k_MaxStackSize) {
    m_startIndex = 0;
  }
  return result;
}
...
Calculation * CalculationStore::push(Poincare::Evaluation<double> * evaluation, Context * context) {
  Calculation * result = &m_calculations[m_startIndex];
  result->setContent(evaluation, context);
  m_startIndex++;
  if (m_startIndex >= k_MaxStackSize) {
    m_startIndex = 0;
  }
...
Calculation * CalculationStore::calculationAtIndex(int i) {
  ...
  while (j <= i) {
    ...
    if (currentCalc >= m_calculations + k_MaxStackSize) {
...
int CalculationStore::numberOfCalculations() {
  ...
  while (currentCalc < m_calculations + k_MaxStackSize) {
    if (!currentCalc++->isEmpty()) {
...
void CalculationStore::deleteCalculationAtIndex(int i) {
  ...
  while (m_calculations[indexFirstCalc].isEmpty()) {
    indexFirstCalc++;
    if (indexFirstCalc == k_MaxStackSize) {
      indexFirstCalc = 0;
    }
    assert(indexFirstCalc != m_startIndex);
  }
  int absoluteIndexCalculationI = indexFirstCalc+i;
  absoluteIndexCalculationI = absoluteIndexCalculationI >= k_MaxStackSize ? absoluteIndexCalculationI - k_MaxStackSize : absoluteIndexCalculationI;

  int index = absoluteIndexCalculationI;
  for (int k = i; k < numberOfCalc-1; k++) {
    int nextIndex = index+1 >= k_MaxStackSize ? 0 : index+1;
    m_calculations[index] = m_calculations[nextIndex];
    index++;
    if (index == k_MaxStackSize) {
      index = 0;
    }
  }
  ...
  m_startIndex--;
  if (m_startIndex == -1) {
    m_startIndex = k_MaxStackSize-1;
  }
}

void CalculationStore::deleteAll() {
  m_startIndex = 0;
  for (int i = 0; i < k_MaxStackSize; i++) {
