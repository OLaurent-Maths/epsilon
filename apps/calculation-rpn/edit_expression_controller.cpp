...
bool EditExpressionController::textFieldDidReceiveEvent(::TextField * textField, Ion::Events::Event event) {
	...
	int nbrOfOperandsOnStack = m_calculationStore->numberOfCalculations();
	int nbrOfOperandsNeeded = 0;
	if (event == Ion::Events::Tangent)
	{
		std::cout << "Tangent typed\n";

		Poincare::Complex<double> argument1 = Poincare::Complex<double>::Float(9);
		Poincare::Expression * args[1] = {&argument1};
		Poincare::Tangent tangent;
		tangent.setArgument(args, 1, true);
		Poincare::Evaluation<double> * result = tangent.evaluate<double>(*calculationApp->localContext());
		m_calculationStore->push(result, calculationApp->localContext());

		// Reload and clear editing
